...
static RMstatus parse_options(CONTEXT* pCtx, const char* szAppName, int argc, char* argv[])
{
    ...
    while ((opt = getopt(argc, argv, opts)) != -1)
    {
        ...
        case 'b':
            if (pCtx->szBinFilename[0] != NULL)
                free((void *)pCtx->szBinFilename[0]);
            pCtx->szBinFilename[0] = strdup(optarg);
            break;

        case 'c':
            if (pCtx->szBinFilename[1] != NULL)
                free((void *)pCtx->szBinFilename[1]);
            pCtx->szBinFilename[1] = strdup(optarg);
            break;

        case 's':
            if (pCtx->szStrFilename != NULL)
                free((void *)pCtx->szStrFilename);
            pCtx->szStrFilename = strdup(optarg);
            break;
        ...
        case 'o':
            ...
            pCtx->szYUVFilename = strdup(optarg);
            break;

        case 'O':
            if (pCtx->szYUVPath != NULL)
                free((void*)pCtx->szYUVPath);
            pCtx->szYUVPath = strdup( optarg );
            break;
    ...
    if ((pCtx->szBinFilename[0] != 0) && (pCtx->szStrFilename != 0))
    {
        if ((file_exists( pCtx->szBinFilename[0] ) == RM_OK) &&
            (file_exists( pCtx->szStrFilename ) == RM_OK))
        {
    ...
    if (result == RM_OK) {
        if ((pCtx->szYUVFilename != 0) && (pCtx->szYUVPath != 0)) {
            fprintf(stderr, "ERROR: Cannot specify -o & -O together!\n");
            result = RM_ERROR;
        } else if (pCtx->szYUVPath != 0) {
            pCtx->szYUVFilename = generate_output_yuv( pCtx->szYUVPath, pCtx->szStrFilename);
        }

        /* if user specified output yuv file, open it for writing... */
        if (pCtx->szYUVFilename != 0)
        {
            pCtx->yuvfp = fopen( pCtx->szYUVFilename, "w");
...
{
    /* microcode bin filename */
    if (pCtx->szBinFilename[0] != 0L)
    {
        free((void*)pCtx->szBinFilename[0]);
        pCtx->szBinFilename[0] = 0L;
    }
    if (pCtx->szBinFilename[1] != 0L)
    {
        free((void*)pCtx->szBinFilename[1]);
        pCtx->szBinFilename[1] = 0L;
    }

    /* input stream filename */
    if (pCtx->szStrFilename != 0L)
    {
        free((void*)pCtx->szStrFilename);
        pCtx->szStrFilename = 0L;
    }
...
//        dramBase    = DRAM_BASE;
        szBinName   = pCtx->szBinFilename[0];
        break;
    case 1:
        pmBase      = PMEM_BASE_mpeg_engine_1;
        dmBase      = DMEM_BASE_mpeg_engine_1;
//        dramBase    = DRAM_BASE;
        szBinName   = pCtx->szBinFilename[1];
#ifdef ENABLE_CURSES
...
//static void do_stop(CONTEXT* pContext)
//{
//    RMuint32    reset_control_reg = pContext->regBaseAddress + G2L_RESET_CONTROL;
//
//#ifndef ENABLE_CURSES
//    printf("Stopping DSP\n");
//    fflush(stdout);
...
//static void do_start(CONTEXT* pContext)
//{
//    RMuint32    reset_control_reg = pContext->regBaseAddress + G2L_RESET_CONTROL;
//
//#ifndef ENABLE_CURSES
...
static int launch_threads(CONTEXT* pCtx)
{
    ...
    SET_DISPLAY_CONTEXT(pCtx, szPlayFile, pCtx->szStrFilename);
    SET_DISPLAY_CONTEXT(pCtx, szCapFile, pCtx->szYUVFilename);
    SET_DISPLAY_CONTEXT(pCtx, szBinFile[0], pCtx->szBinFilename[0]);
...
//    DISPLAY_CTX_PTR( uiDRAMPtr );
    ...
    DISPLAY_CTX_STR( szBinFilename[0], "Microcode Binary #0");
